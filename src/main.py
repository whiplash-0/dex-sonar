...
class Application:
    ...
    def _callback_on_price_update(self, pair: Pair):
        if spike := self.spike_detector.detect(pair):
            logger.info(f'{pair.base_symbol + ":":>{pair.MAX_BASE_COIN_LEN + 1}} {spike.change:+.1%}')
    ...
    async def task_call_async_callbacks_from_live_pairs(self):
        while True:
            ...
            logger.debug(f'{pair.base_symbol + ":":>{pair.MAX_BASE_COIN_LEN + 1}} Delay before callback: {time.get_monotonic() - start_time:.1f}s'); start_time = time.get_monotonic()
            await self._callback_on_price_update_async(pair, spike)
            logger.debug(f'{pair.base_symbol + ":":>{pair.MAX_BASE_COIN_LEN + 1}} Callback executed in:  {time.get_monotonic() - start_time:.1f}s. Left: {self.callback_queue.qsize()}')
